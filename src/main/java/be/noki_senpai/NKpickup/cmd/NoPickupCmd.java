...
public class NoPickupCmd implements CommandExecutor
{
	...
	@Override public boolean onCommand(CommandSender sender, org.bukkit.command.Command command, String label, String[] args)
	{
		...
		{
			if(!(sender.hasPermission("*") || sender.hasPermission("nkpickup.*") || sender.hasPermission("nkpickup.nopickup")
				...
				else
				{
					if(NKpickup.players.get(sender.getName()).getBlackList().size() < NKpickup.maxList)
					{
						if(NKpickup.material.contains(args[0].toString().toLowerCase()))
						{
							...
							sender.sendMessage(ChatColor.GREEN + " L'item " + ChatColor.BOLD + args[0].toString().toLowerCase() + ChatColor.RESET
									+ ChatColor.GREEN + " a été ajouté à votre blacklist.");
							return true;
						}
						else
						{
					...
					{
						sender.sendMessage(ChatColor.RED + " Vous ne pouvez pas avoir plus de " + ChatColor.BOLD + NKpickup.maxList + ChatColor.RESET
								+ ChatColor.RED + " item(s) dans votre liste.");
						return true;
					}
				}
			}
